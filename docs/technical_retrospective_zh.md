...
这个项目的核心原型其实在 2017 年就已经做成了，只是直到最近才被重新整理成论文、文档和开源仓库。现在回头看，它最可惜的地方不是代码晚开源，而是当年没有及时把这个交叉点讲清楚：动态内存管理、字典树和 FastText n-gram 统计现象，本来可以形成一个很完整的工程研究故事。
...
这个项目最让我遗憾的地方，是它其实在 2017 年就已经有了清晰的技术内核：动态内存管理、字典树、FastText n-gram 语言统计现象，这三件事接在一起，形成的是一个非常完整的工程研究问题。
